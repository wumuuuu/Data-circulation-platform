...
public interface STUMapper {
    ...
    @Update("UPDATE signtask_user SET status = #{status}, y = #{y}, b = #{b}, b1 = #{b1} " +
            "WHERE task_id = #{taskId} AND user_name = #{userName}")
    void updateStatus(int taskId, String userName, String status, String y, String b, String b1);
    ...
    // 根据 taskId 和 username 查找 b1
    @Select("SELECT b1 FROM SIGNTASK_USER WHERE task_id = #{taskId} AND user_name = #{username}")
    String findB1(int taskId, String username);
